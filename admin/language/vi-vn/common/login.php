<?php
// Heading
$_['heading_title']  = 'Quản trị';

// Text
$_['text_heading']   = 'Quản trị';
$_['text_login']     = 'Vui lòng nhập thông tin đăng nhập của bạn.';
$_['text_forgotten'] = 'Quên mật khẩu';

// Entry
$_['entry_username'] = 'Tên đăng nhập';
$_['entry_password'] = 'Mật khẩu';

// Button
$_['button_login']   = 'Đăng nhập';

// Error
$_['error_login']    = 'Tên đăng nhập và/hoặc Mật khẩu không đúng.';
$_['error_token']    = 'Phiên token không hợp lệ. Vui lòng đăng nhập lại.';
$_['error_attempts'] = 'Cảnh báo: Tài khoản của bạn đã vượt quá số lần đăng nhập cho phép. Vui lòng thử lại sau 1 giờ hoặc đặt lại mật khẩu.';
